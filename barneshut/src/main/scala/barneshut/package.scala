...
package object barneshut {
  ...
  case class Fork(
    ...
    val massX: Float = if (mass==0f) { centerX}
    else {quadList.foldLeft(0f)((sum, quad) => (sum + quad.mass * quad.massX))/mass}
    val massY: Float = if (mass==0f) { centerY}
  ...
  extends Quad {
    val (mass, massX, massY) = (bodies.foldLeft(0f)(_ + _.mass) : Float,
      bodies.foldLeft(0f)( (sum, body) => (sum + body.mass*body.x)/body.mass): Float,
      bodies.foldLeft(0f)( (sum, body) => (sum + body.mass*body.y)/body.mass) : Float)
    val total: Int = bodies.length
    def insert(b: Body): Quad = if (total>minimumSize) {
  ...
  class Body(val mass: Float, val x: Float, val y: Float, val xspeed: Float, val yspeed: Float) {
    ...
    def updated(quad: Quad): Body = {
      ...
      //calculates how much force the Body takes from a quad (Empty, Leaf or Fork)
